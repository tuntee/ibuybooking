...
// services route
    $app->post('/account/service/:action/?', function ($action) use ($app) {
        $controller = new UF\AccountControllerService($app);
    ...
    // User info json data for service
    $app->get('/users/u/:user_id/:token?', function ($user_id, $secret_token) use ($app) {
        $controller = new UF\UserController($app);
        return $controller->getUser($user_id, $secret_token);
    });
...
$app->get('/companies/?', function () use ($app) {
    $controller = new UF\CompanyController($app);
    return $controller->pageCompanies();
});

// Company info page
$app->get('/company/c/?', function () use ($app) {
    $controller = new UF\CompanyController($app);
    return $controller->pageCompany();
});
...
//// List companies in a particular user id
//$app->get('/companies/:primary_group/?', function ($primary_group) use ($app) {
//    $controller = new UF\CompanyController($app);
//    return $controller->pageUsers($primary_group);
//});

// company info form (update/view)
$app->get('/forms/company/c/:company_id/?', function ($company_id) use ($app) {
    $controller = new UF\CompanyController($app);
    $get = $app->request->get();
    if (isset($get['mode']) && $get['mode'] == "update")
        return $controller->formCompanyEdit($company_id);
    else
        return $controller->formCompanyView($company_id);
});

// company creation form
$app->get('/forms/companies/?', function () use ($app) {
    $controller = new UF\CompanyController($app);
    return $controller->formGroupCreate();
});
...
$app->post('/company/?', function () use ($app) {
    $controller = new UF\CompanyController($app);
    return $controller->createCompany();
});

// Update company info
$app->post('/company/c/:company_id/?', function ($company_id) use ($app) {
    $controller = new UF\CompanyController($app);
    return $controller->updateCompany($company_id);
});

// Delete company
$app->post('/company/g/:company_id/delete/?', function ($company_id) use ($app) {
    $controller = new UF\CompanyController($app);
    return $controller->deleteCompany($company_id);
});

//$app->get('/company/info/:company_id/?', function ($company_id) use ($app) {
//    echo "Company info page by user $company_id";
//    $app->render('company-info.twig', [
//        'company' => $app->company->getType()
//    ]);
//});

// ------------------------ END company route -----------------------------------------------

// ------------------------ START company config route -----------------------------------------------
// Company info page
$app->get('/company_config/c/?', function () use ($app) {
    $controller = new UF\CompanyConfigController($app);
    return $controller->pageCompany();
});

// company info form (update/view)
$app->get('/forms/company_config/c/:company_id/?', function ($company_id) use ($app) {
    $controller = new UF\CompanyConfigController($app);
    $get = $app->request->get();
    if (isset($get['mode']) && $get['mode'] == "update")
        return $controller->formCompanyEdit($company_id);
...
//// Create company
//$app->post('/company_config/?', function () use ($app) {
//    $controller = new UF\CompanyController($app);
//    return $controller->createCompany();
//});

// Update company info
$app->post('/company_config/c/:company_id/?', function ($company_id) use ($app) {
    $controller = new UF\CompanyConfigController($app);
    return $controller->updateCompany($company_id);
});

// Delete company
$app->post('/company_config/g/:company_id/delete/?', function ($company_id) use ($app) {
    $controller = new UF\CompanyConfigController($app);
    return $controller->deleteCompany($company_id);
});


// ------------------------ END company config route -----------------------------------------------

/********** BRANCH MANAGEMENT INTERFACE **********/

// List companies
$app->get('/branches/?', function () use ($app) {
    $controller = new UF\BranchController($app);
    return $controller->pageBranches();
});

// Company info page
$app->get('/branch/b/:branch_id?', function ($branch_id) use ($app) {
    $controller = new UF\BranchController($app);
    return $controller->pageBranch($branch_id);
});
...
// branch info form (update/view)
$app->get('/forms/branch/b/:branch_id/?', function ($branch_id) use ($app) {
    $controller = new UF\BranchController($app);
    $get = $app->request->get();
    if (isset($get['mode']) && $get['mode'] == "update")
        return $controller->formBranchEdit($branch_id);
    else
        return $controller->formBranchView($branch_id);
});

// branch creation form
$app->get('/forms/branches/?', function () use ($app) {    
    $controller = new UF\BranchController($app);
    return $controller->formBranchCreate();
});

// Create branch
$app->post('/branches/?', function () use ($app) {
    $controller = new UF\BranchController($app);
    return $controller->createBranch();
});

// Update branch info
$app->post('/branch/b/:branch_id/?', function ($branch_id) use ($app) {
    $controller = new UF\BranchController($app);
    return $controller->updateBranch($branch_id);
});

// Delete branch
$app->post('/branch/b/:branch_id/delete/?', function ($branch_id) use ($app) {
    $controller = new UF\BranchController($app);
    return $controller->deleteBranch($branch_id);
});
...
$app->get('/services/?', function () use ($app) {
    $controller = new UF\ServiceController($app);
    return $controller->pageServices();
});

// Company info page
$app->get('/service/b/:service_id?', function ($service_id) use ($app) {
    $controller = new UF\ServiceController($app);
    return $controller->pageService($service_id);
});
...
// branch info form (update/view)
$app->get('/forms/service/b/:service_id/?', function ($service_id) use ($app) {
    $controller = new UF\ServiceController($app);
    $get = $app->request->get();
    if (isset($get['mode']) && $get['mode'] == "update")
        return $controller->formServiceEdit($service_id);
    else
        return $controller->formServiceView($service_id);
});

// branch creation form
$app->get('/forms/services/?', function () use ($app) {    
    $controller = new UF\ServiceController($app);
    return $controller->formServiceCreate();
});
...
// Update branch info
$app->post('/service/b/:service_id/?', function ($service_id) use ($app) {
    $controller = new UF\ServiceController($app);
    return $controller->updateService($service_id);
});

// Delete branch
$app->post('/service/b/:service_id/delete/?', function ($service_id) use ($app) {
    $controller = new UF\ServiceController($app);
    return $controller->deleteService($service_id);
});
...
$app->get('/promotions/?', function () use ($app) {
    $controller = new UF\PromotionController($app);
    return $controller->pagePromotions();
});

// Company info page
$app->get('/promotion/b/:promotion_id?', function ($promotion_id) use ($app) {
    $controller = new UF\PromotionController($app);
    return $controller->pagePromotion($promotion_id);
});

// branch info form (update/view)
$app->get('/forms/promotion/b/:promotion_id/?', function ($promotion_id) use ($app) {
    $controller = new UF\PromotionController($app);
    $get = $app->request->get();
    if (isset($get['mode']) && $get['mode'] == "update")
        return $controller->formPromotionEdit($promotion_id);
    else
        return $controller->formPromotionView($promotion_id);
});

// branch creation form
$app->get('/forms/promotions/?', function () use ($app) {
    $controller = new UF\PromotionController($app);
    return $controller->formPromotionCreate();
});

// Create branch
$app->post('/promotions/?', function () use ($app) {
    $controller = new UF\PromotionController($app);
    return $controller->createPromotion();
});

// Update branch info
$app->post('/promotion/b/:promotion_id/?', function ($promotion_id) use ($app) {
    $controller = new UF\PromotionController($app);
    return $controller->updatePromotion($promotion_id);
});

// Delete branch
$app->post('/promotion/b/:promotion_id/delete/?', function ($promotion_id) use ($app) {
    $controller = new UF\PromotionController($app);
    return $controller->deletePromotion($promotion_id);
});
...
//    $controller = new UF\CompanyController($app);
//    echo "test";
    $app->render('companies/summary.twig');
//    return $controller->pageCompanies();
});
