...
function bindUserTableButtons(table) {
    // Link buttons
    $(table).find('.js-service-create').click(function() {
        userForm('dialog-service-create');
    });
    
    $(table).find('.js-service-edit').click(function() {
        var btn = $(this);
        var user_id = btn.data('id');
        userForm('dialog-service-edit', user_id);
    });
...
//    });
    ...
    $(table).find('.js-service-enable').click(function () {
        var btn = $(this);
        var user_id = btn.data('id');
    ...
    $(table).find('.js-service-disable').click(function () {
        var btn = $(this);
        var user_id = btn.data('id');
    ...
    $(table).find('.js-service-delete').click(function() {
        var btn = $(this);
        var user_id = btn.data('id');
        var user_name = btn.data('service_name');
        deleteUserDialog('dialog-service-delete', user_id, user_name);
    });	 	        
}
...
function updateUserEnabledStatus(user_id, flag_enabled) {
	...
	var url = site['uri']['public'] + "/service/b/" + user_id;
...
function deleteUserDialog(box_id, user_id, name){
	...
	.done(function(result) {		
		...
		$('#' + box_id + ' .js-confirm').click(function(){
            ...
            var url = site['uri']['public'] + "/service/b/" + user_id + "/delete";
...
function userForm(box_id, user_id) {	
    ...
    var url = site['uri']['public'] + "/forms/services";
    ...
    if (user_id) {
        ...
        url = site['uri']['public'] + "/forms/service/b/" + user_id;
    }
...
function userPasswordForm(box_id, user_id) {	
    ...
    var url = site['uri']['public'] + "/forms/service/b/" + user_id + "/password";
...
function userDisplay(box_id, user_id) {
	...
	.done(function(result) {
		...
		// Link buttons
		$('#' + box_id + ' .js-service-edit').click(function() {
			userForm('dialog-service-edit', user_id);
		});
...
//		});
		...
		$('#' + box_id + ' .js-service-enable').click(function () {
			updateUserEnabledStatus(user_id, "1");
		});
		
		$('#' + box_id + ' .js-service-disable').click(function () {
			updateUserEnabledStatus(user_id, "0");
		});	
		
		$('#' + box_id + ' .js-service-delete').click(function() {
			var user_name = $(this).data('name');
			deleteUserDialog('delete-service-dialog', user_id, user_name);
			$('#dialog-service-delete').modal('show');
		});	
