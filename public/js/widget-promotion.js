...
function bindUserTableButtons(table) {
    // Link buttons
    $(table).find('.js-promotion-create').click(function() {
        userForm('dialog-promotion-create');
    });
    
    $(table).find('.js-promotion-edit').click(function() {
        var btn = $(this);
        var user_id = btn.data('id');
        userForm('dialog-promotion-edit', user_id);
    });
...
//    });
    ...
    $(table).find('.js-promotion-enable').click(function () {
        var btn = $(this);
        var user_id = btn.data('id');
    ...
    $(table).find('.js-promotion-disable').click(function () {
        var btn = $(this);
        var user_id = btn.data('id');
    ...
    $(table).find('.js-promotion-delete').click(function() {
        var btn = $(this);
        var user_id = btn.data('id');
        var user_name = btn.data('promotion_title');
        deleteUserDialog('dialog-promotion-delete', user_id, user_name);
    });	 	        
}
...
function updateUserEnabledStatus(user_id, flag_enabled) {
	...
	var url = site['uri']['public'] + "/promotion/b/" + user_id;
...
function deleteUserDialog(box_id, user_id, name){
	...
	.done(function(result) {		
		...
		$('#' + box_id + ' .js-confirm').click(function(){
            ...
            var url = site['uri']['public'] + "/promotion/b/" + user_id + "/delete";
...
function userForm(box_id, user_id) {	
    ...
    var url = site['uri']['public'] + "/forms/promotions";
    ...
    if (user_id) {
        ...
        url = site['uri']['public'] + "/forms/promotion/b/" + user_id;
    }
...
function userPasswordForm(box_id, user_id) {	
    ...
    var url = site['uri']['public'] + "/forms/promotion/b/" + user_id + "/password";
...
function userDisplay(box_id, user_id) {
	...
	.done(function(result) {
		...
		// Link buttons
		$('#' + box_id + ' .js-promotion-edit').click(function() {
			userForm('dialog-promotion-edit', user_id);
		});
...
//		});
		...
		$('#' + box_id + ' .js-promotion-enable').click(function () {
			updateUserEnabledStatus(user_id, "1");
		});
		
		$('#' + box_id + ' .js-promotion-disable').click(function () {
			updateUserEnabledStatus(user_id, "0");
		});	
		
		$('#' + box_id + ' .js-promotion-delete').click(function() {
			var user_name = $(this).data('name');
			deleteUserDialog('delete-promotion-dialog', user_id, user_name);
			$('#dialog-promotion-delete').modal('show');
		});	
