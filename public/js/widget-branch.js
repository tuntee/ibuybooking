...
function bindUserTableButtons(table) {
    // Link buttons
    $(table).find('.js-branch-create').click(function() {
        userForm('dialog-branch-create');
    });
    
    $(table).find('.js-branch-edit').click(function() {
        var btn = $(this);
        var user_id = btn.data('id');
        userForm('dialog-branch-edit', user_id);
    });

//    $(table).find('.js-branch-password').click(function() {
//        var btn = $(this);
//        var user_id = btn.data('id');
//        userPasswordForm('dialog-branch-password', user_id);
//    });

//    $(table).find('.js-branch-activate').click(function() {
//        var btn = $(this);
//        var user_id = btn.data('id');
...
//    });
    ...
    $(table).find('.js-branch-enable').click(function () {
        var btn = $(this);
        var user_id = btn.data('id');
    ...
    $(table).find('.js-branch-disable').click(function () {
        var btn = $(this);
        var user_id = btn.data('id');
    ...
    $(table).find('.js-branch-delete').click(function() {
        var btn = $(this);
        var user_id = btn.data('id');
        var user_name = btn.data('branch_name');
        deleteUserDialog('dialog-branch-delete', user_id, user_name);
    });	 	        
}
...
function updateUserEnabledStatus(user_id, flag_enabled) {
	...
	var url = site['uri']['public'] + "/branch/b/" + user_id;
...
function deleteUserDialog(box_id, user_id, name){
	...
	var data = {
		box_id: box_id,
		box_title: "Delete Branch",
		confirm_message: "Are you sure you want to delete the branch " + name + "?",
		confirm_button: "Yes, delete branch"
	};
	...
	.done(function(result) {		
		...
		$('#' + box_id + ' .js-confirm').click(function(){
            ...
            var url = site['uri']['public'] + "/branch/b/" + user_id + "/delete";
...
function userForm(box_id, user_id) {	
    ...
    var url = site['uri']['public'] + "/forms/branches";  
    ...
    if (user_id) {
        ...
        url = site['uri']['public'] + "/forms/branch/b/" + user_id;
    }
...
function userPasswordForm(box_id, user_id) {	
    ...
    var url = site['uri']['public'] + "/forms/branch/b/" + user_id + "/password";
...
function userDisplay(box_id, user_id) {
	...
	.done(function(result) {
		...
		// Link buttons
		$('#' + box_id + ' .js-branch-edit').click(function() {
			userForm('dialog-branch-edit', user_id);
		});
...
//		});
		...
		$('#' + box_id + ' .js-branch-enable').click(function () {
			updateUserEnabledStatus(user_id, "1");
		});
		
		$('#' + box_id + ' .js-branch-disable').click(function () {
			updateUserEnabledStatus(user_id, "0");
		});	
		
		$('#' + box_id + ' .js-branch-delete').click(function() {
			var user_name = $(this).data('name');
			deleteUserDialog('delete-branch-dialog', user_id, user_name);
			$('#dialog-branch-delete').modal('show');
		});	
