<?php
/**
 * initialization file.  Handles setup for database, site settings, JS/CSS includes, etc.
 *
 */
...
if (!file_exists(__DIR__ . "/config-userfrosting.php")){
    echo "<h2>We can't seem to find config-userfrosting.php!  You should rename the file config-userfrosting-example.php to config-userfrosting.php, and then fill in the configuration details for your database and server.</h2><br>";
    exit;
}
...
$table_company = new \UserFrosting\DatabaseTable($app->config('db')['db_prefix'] . "company_info", [
    "id",
    "company_name_th",
    "company_name_en",
    "company_name_cn",
...
$table_company_user = new \UserFrosting\DatabaseTable($app->config('db')['db_prefix'] . "company_user", [
    "id",
    "user_id",
    "company_id"
]);

$table_branch = new \UserFrosting\DatabaseTable($app->config('db')['db_prefix'] . "branch", [
    "id",
    "company_id",
    "name_th",
    "name_en",
...
$table_service = new \UserFrosting\DatabaseTable($app->config('db')['db_prefix'] . "services", [
    "id",
    "company_id",
    "name_th",
    "name_en",
    ...
    "duration",
    "customer_quantity",
    "max_customer_support",
    "created_at",
    "updated_at",
    "status"
]);

$table_company_config = new \UserFrosting\DatabaseTable($app->config('db')['db_prefix'] . "company_config", [
    "id",
    "company_id",
    "payment_option_online",
    "payment_option_offline",
    "accept_all_booking",
    "time_cancel_reservation",
    "time_offer_booking",
    "allow_edit_order",
    "allow_refund",
    "auto_send_sale_report",
    "auto_delete_and_export",
    "keep_sale_summary",
    "updated_at"
]);

$table_promotion = new \UserFrosting\DatabaseTable($app->config('db')['db_prefix'] . "promotion", [
    "id",
    "service_id",
    "title_th",
    "title_en",
...
$table_branch_service = new \UserFrosting\DatabaseTable($app->config('db')['db_prefix'] . "branch_service");
$table_service_promotion = new \UserFrosting\DatabaseTable($app->config('db')['db_prefix'] . "service_promotion");
...
\UserFrosting\Database::setSchemaTable("company_info", $table_company);
\UserFrosting\Database::setSchemaTable("company_user", $table_company_user);
\UserFrosting\Database::setSchemaTable("branch", $table_branch);
\UserFrosting\Database::setSchemaTable("branch_service", $table_branch_service);
\UserFrosting\Database::setSchemaTable("service_promotion", $table_service_promotion);
\UserFrosting\Database::setSchemaTable("service", $table_service);
\UserFrosting\Database::setSchemaTable("company_config", $table_company_config);
\UserFrosting\Database::setSchemaTable("promotion", $table_promotion);
...
$setting_values = [
    'userfrosting' => [
        ...
        'admin_email' => 'admin@ibuybooking.com',
        'email_login' => '1',
        'can_register' => '1',
        'enable_captcha' => '1',
...
$app->hook('includes.css.register', function () use ($app){
    ...
    // Summary CSS
    $app->schema->registerCSS("summary", "timeline.css");
    $app->schema->registerCSS("summary", "lib/morris.css");
    $app->schema->registerCSS("summary", "http://code.jquery.com/ui/1.11.4/themes/smoothness/jquery-ui.css");
...
$app->hook('includes.js.register', function () use ($app){
    ...
    // Summary JS
    $app->schema->registerJS("summary", "lib/raphael.js");
    $app->schema->registerJS("summary", "lib/morris.js");

    // Users JS
    $app->schema->registerJS("user", "widget-users.js");

    // Company JS
    $app->schema->registerJS("company", "widget-company.js");

    // Company JS
    $app->schema->registerJS("company_config", "widget-company-config.js");

    // Branch JS
    $app->schema->registerJS("branch", "widget-branch.js");

    // Service JS
    $app->schema->registerJS("service", "widget-service.js");

    // Promotion JS
    $app->schema->registerJS("promotion", "widget-promotion.js");
    $app->schema->registerJS("promotion", "jquery-ui.min.js");
    $app->schema->registerCSS("promotion", "jquery-ui.min.css");
    $app->schema->registerCSS("promotion", "jquery-ui.structure.min.css");
    $app->schema->registerCSS("promotion", "jquery-ui.theme.min.css");
