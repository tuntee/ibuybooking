...
class CompanyLoader {
    ...
    public static function exists($value, $name = "id"){
        if ($name == "id")
            // Fetch by id
            return ( Company::find($value) ? true : false );
        else
            // Fetch by some other column name
            return ( Company::where($name, $value)->first() ? true : false );
    }
    ...
    public static function fetch($value, $name = "id"){
        if ($name == "id")
            ...
            // Fetch by some other column name
            return Company::where($name, $value)->first();
    }
    ...
    public static function fetchAll($value = null, $name = null){
        if (!$value || !$name)
            ...
            $result = Company::where($name, $value)->get();
