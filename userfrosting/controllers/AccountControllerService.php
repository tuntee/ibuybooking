 ...
 */
class AccountControllerService extends \UserFrosting\BaseController {
...
//            $this->_app->halt(400);
            $msg_type = "danger";
            $msg_code = "VALIDATE_FAIL";
            $msg = "Validation fail";
            $this->_app->halt_s(400, $msg_code, $msg, $msg_type);
        }
...
//            $ms->addMessageTranslated("danger", "ACCOUNT_USER_OR_PASS_INVALID");
            $msg_type = "danger";
            $msg_code = "ACCOUNT_USER_OR_PASS_INVALID";
            $msg = $ms->getMessageTranslated("danger", "ACCOUNT_USER_OR_PASS_INVALID");
            $this->_app->halt_s(403, $msg_code, $msg, $msg_type);
        }
...
//                $ms->addMessageTranslated("danger", "ACCOUNT_USER_OR_PASS_INVALID");
            ...
            if(!$user->isGuest()) {
                $msg = $ms->getMessageTranslated("warning", "LOGIN_ALREADY_COMPLETE");
                $this->_app->halt_s(200, "LOGIN_ALREADY_COMPLETE", $msg, "warning");
            }
...
//            $ms->addMessageTranslated("danger", "ACCOUNT_DISABLED");
            $msg_type = "danger";
            $msg_code = "ACCOUNT_DISABLED";
            $msg = $ms->getMessageTranslated("danger", "ACCOUNT_DISABLED");
            $this->_app->halt_s(403, $msg_code, $msg, $msg_type);
        }
...
//            $ms->addMessageTranslated("danger", "ACCOUNT_INACTIVE");
            $msg_type = "danger";
            $msg_code = "ACCOUNT_INACTIVE";
            $msg = $ms->getMessageTranslated("danger", "ACCOUNT_INACTIVE");
            $this->_app->halt_s(403, $msg_code, $msg, $msg_type);
        }

        // Here is my password.  May I please assume the identify of this user now?
        if ($user->verifyPassword($data['password']))  {
            $this->_app->login($user, !empty($data['rememberme']), true);
...
//            $ms->addMessage("danger", "Aww hellllls no!");
            $this->_app->halt_s(500, "HI_BOT", "Aww hellllls no!", "danger");     // Don't let on about why the request failed ;-)
        }
        ...
        if (!User::find($this->_app->config('user_id_master'))){
//            $ms->addMessageTranslated("danger", "MASTER_ACCOUNT_NOT_EXISTS");
            $msg = $ms->getMessageTranslated("danger", "MASTER_ACCOUNT_NOT_EXISTS");
            $this->_app->halt_s(403, "MASTER_ACCOUNT_NOT_EXISTS", $msg, "danger");
        }

        // Check if registration is currently enabled
        if (!$this->_app->site->can_register){
//            $ms->addMessageTranslated("danger", "ACCOUNT_REGISTRATION_DISABLED");
            $msg = $ms->getMessageTranslated("danger", "ACCOUNT_REGISTRATION_DISABLED");
            $this->_app->halt_s(403, "ACCOUNT_REGISTRATION_DISABLED", $msg, "danger");
        }

        // Prevent the user from registering if he/she is already logged in
        if(!$this->_app->user->isGuest()) {
            $msg = $ms->getMessageTranslated("danger", "ACCOUNT_REGISTRATION_LOGOUT");
            $this->_app->halt_s(200, "ACCOUNT_REGISTRATION_LOGOUT", $msg, "logged_in");
        }
        ...
        if ($error) {
            $this->_app->halt_s(400, "ACCOUNT_REGISTRATION_ERROR", "Any validation errors", "danger");
        }
...
//            $ms->addMessageTranslated("danger", "ACCOUNT_REGISTRATION_BROKEN");
            $msg = $ms->getMessageTranslated("danger", "ACCOUNT_REGISTRATION_BROKEN");
            error_log("Account registration is not working because a default primary group has not been set.");
            $this->_app->halt_s(500, "ACCOUNT_DEFAULT_PRIMARYGROUP_NOT_SET", $msg, "danger");
        }
...
//                $ms->addMessageTranslated("danger", "MAIL_ERROR");
                $msg = $ms->getMessageTranslated("danger", "MAIL_ERROR");
                error_log('Mailer Error: ' . $e->errorMessage());
                $this->_app->halt_s(500, "MAIL_ERROR", $msg, "danger");
            }
            $msg_code = "ACCOUNT_REGISTRATION_COMPLETE_TYPE2";
            $msg = $ms->getMessageTranslated("success", "ACCOUNT_REGISTRATION_COMPLETE_TYPE2");
        } else
            // No activation required
            $msg = $ms->getMessageTranslated("success", "ACCOUNT_REGISTRATION_COMPLETE_TYPE1");
            $msg_code = "ACCOUNT_REGISTRATION_COMPLETE_TYPE1";

        // Return the user object to the calling program
        // Return json response to service
        $result = $this->_app->halt_s(200, $msg_code, $msg, "success");
        return $result;
    }
    ...
    public function forgotPassword(){
        ...
        if(!$user) {
            $msg = $ms->getMessageTranslated("success", "FORGOTPASS_REQUEST_SUCCESS");
            $this->_app->halt_s(200, "FORGOTPASS_REQUEST_SUCCESS", $msg, "success");
        }
...
//        }
        ...
        $user->save();
        $msg = $ms->getMessageTranslated("success", "FORGOTPASS_REQUEST_SUCCESS");
        $result = $this->_app->halt_s(200, "FORGOTPASS_REQUEST_SUCCESS", $msg, "success");
        return $result;
    }
    ...
    public function setPassword($flag_new_user = false){
        ...
        if (!$user){
            $msg = $ms->getMessageTranslated("danger", "FORGOTPASS_INVALID_TOKEN");
            $this->_app->halt_s(400, "FORGOTPASS_INVALID_TOKEN", $msg, "danger");
        }
        ...
        if ($user->flag_password_reset == 0 || $last_password_reset_time === null){
            $msg = $ms->getMessageTranslated("danger", "FORGOTPASS_INVALID_TOKEN");
            $this->_app->halt_s(400, "FLAG_PASSWORD_RESET_OR_TIME_ERROR", "Check that a lost flag password reset request is in progress and has not expired", "danger");
        }
...
//            $msg = $ms->getMessageTranslated("danger", "FORGOTPASS_OLD_TOKEN");
//            $this->_app->halt_s(400, "FORGOTPASS_OLD_TOKEN", $msg, "danger");
//        }
...
//			$ms->addMessageTranslated("danger", "PASSWORD_HASH_FAILED");
			$msg = $ms->getMessageTranslated("danger", "PASSWORD_HASH_FAILED");
            $this->_app->halt_s(500, "PASSWORD_HASH_FAILED", $msg, "danger");
		}
        ...
        $ms = $this->_app->alerts;
        $ms->addMessageTranslated("success", "ACCOUNT_WELCOME", $this->_app->user->export());
        $msg = $ms->getMessageTranslated("success", "ACCOUNT_PASSWORD_UPDATED");

//        $result = $this->_app->halt_s(200, "ACCOUNT_PASSWORD_UPDATED", $msg, "success");

        $result = ["status"=>200,
            "message_code"=>"ACCOUNT_PASSWORD_UPDATED",
            "message"=>$msg,
            "type"=>"success",
            "new_secret_token"=>$this->_app->user->secret_token];
        return $result;
    }
    ...
    public function resendActivation(){
        ...
        // Validate
        if (!$rf->validate()) {
            $this->_app->halt_s(400, "ACCOUNT_RESENDACTIVATION_ERROR", "Any validation errors", "danger");
        }
...
//            $ms->addMessageTranslated("danger", "ACCOUNT_INVALID_USERNAME");
            $msg = $ms->getMessageTranslated("danger", "ACCOUNT_INVALID_USERNAME");
            $this->_app->halt_s(400, "ACCOUNT_INVALID_USERNAME", $msg, "danger");
        }
...
//            $ms->addMessageTranslated("danger", "ACCOUNT_USER_OR_EMAIL_INVALID");
            $msg = $ms->getMessageTranslated("danger", "ACCOUNT_USER_OR_EMAIL_INVALID");
            $this->_app->halt_s(400, "ACCOUNT_USER_OR_EMAIL_INVALID", $msg, "danger");
        }

        // Check if user's account is already active
        if ($user->flag_verified == "1") {
            $msg = $ms->getMessageTranslated("danger", "ACCOUNT_ALREADY_ACTIVE");
            $this->_app->halt_s(400, "ACCOUNT_ALREADY_ACTIVE", $msg, "danger");
        }
...
//          $ms->addMessageTranslated("danger", "ACCOUNT_LINK_ALREADY_SENT", ["resend_activation_threshold" => $this->_app->site->resend_activation_threshold]);
            $msg = $ms->getMessageTranslated("danger", "ACCOUNT_LINK_ALREADY_SENT", ["resend_activation_threshold" => $this->_app->site->resend_activation_threshold]);
            $this->_app->halt_s(429, "ACCOUNT_LINK_ALREADY_SENT", $msg, "danger"); // "Too many requests" code (http://tools.ietf.org/html/rfc6585#section-4)
        }
...
//            $ms->addMessageTranslated("danger", "MAIL_ERROR");
        ...
        $user->save();
        $msg = $ms->getMessageTranslated("success", "ACCOUNT_NEW_ACTIVATION_SENT");
        $result = $this->_app->halt_s(200, "ACCOUNT_NEW_ACTIVATION_SENT", $msg, "success");
        return $result;
    }
...
//        if (!$this->_app->user->checkAccess('uri_account_settings')){
//            $msg = $ms->getMessageTranslated("danger", "ACCESS_DENIED");
//            $this->_app->halt_s(403, "ACCESS_DENIED", $msg, "danger");
//        }
        ...
        // Fetch the user, by looking up the submitted secret token and user_name
        $user = User::where('user_name', $data['user_name'])->where('secret_token', $data['secret_token'])->first();

        // If no user exists for this token.
        if (!$user){
            $msg = $ms->getMessageTranslated("danger", "ACCESS_DENIED");
            $this->_app->halt_s(400, "ACCESS_DENIED", $msg, "danger");
        }else{
            $this->_app->user = $user;
        }
        ...
        if($this->_app->user->isGuest()) {
//            $msg = $ms->getMessageTranslated("warning", "LOGIN_ALREADY_COMPLETE");
            $this->_app->halt_s(400, "LOGIN_NOT_COMPLETE", "Please login.", "danger");
        }

        // Check current password
        if (!isset($data['passwordcheck']) || !$this->_app->user->verifyPassword($data['passwordcheck'])){
            $msg = $ms->getMessageTranslated("danger", "ACCOUNT_PASSWORD_INVALID");
            $this->_app->halt_s(403, "ACCOUNT_PASSWORD_INVALID", $msg, "danger");
        }
        ...
        if (isset($data['email']) && $data['email'] != $this->_app->user->email){
            ...
            if (User::where('email', $data['email'])->first()){
                $msg = $ms->getMessageTranslated("danger", "ACCOUNT_EMAIL_IN_USE", $data);
                $this->_app->halt_s(403, "ACCOUNT_EMAIL_IN_USE", $msg, "danger");
            }
        } else {
        ...
        if (isset($data['locale']) && $data['locale'] != $this->_app->user->locale){
            ...
            if (!in_array($data['locale'], $this->_app->site->getLocales())){
                $msg = $ms->getMessageTranslated("danger", "ACCOUNT_SPECIFY_LOCALE");
                $this->_app->halt_s(403, "ACCOUNT_SPECIFY_LOCALE", $msg, "danger");
            }
        } else {
        ...
        $msg = $ms->getMessageTranslated("success", "ACCOUNT_SETTINGS_UPDATED");
        $result = ["status"=>200,
            "message_code"=>"ACCOUNT_SETTINGS_UPDATED",
            "message"=>$msg,
            "type"=>"success"
//            "new_secret_token"=>$this->_app->user->secret_token
        ];
