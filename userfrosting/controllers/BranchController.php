 ...
 */
class BranchController extends \UserFrosting\BaseController {
     ...
     */
    public function pageBranch($branch_id){
        ...
        if (!$this->_app->user->checkAccess('uri_branch') && $this->_app->user->checkAccess("view_branch_setting")){
            $this->_app->notFound();
        }
...
//            $template = "components/common/company-info-panel.twig";
        ...
        foreach ($fields as $field){
            if ($this->_app->user->checkAccess("view_branch_setting", [ "property" => $field]))
        ...
        $schema = new \Fortress\RequestSchema($this->_app->config('schema.path') . "/forms/branch-create.json");
        $this->_app->jsValidator->setSchema($schema);

        $this->_app->render('branch/branch-info.twig', [
            "box_id" => 'view-branch',
            "alerts_id" => 'form-view-branch-alerts',
            "branch" => $branch,
            "fields" => [
    ...
    public function pageBranches(){
       ...
       if ($this->_app->user->primary_group_id != 2){
        ...
        $this->_app->render('branch/branches.twig', [
            "branches" => $branches
        ]);
    }

    public function formBranchEdit($branch_id){
...
//        $target_user = User::find($user_id);
        ...
        // Access-controlled resource
        if (!$this->_app->user->checkAccess('uri_branch') && $this->_app->user->checkAccess("edit_branch_setting", ["self_company" => $branch])){
            $this->_app->notFound();
        }
...
//        }
        ...
        if ($render == "modal")
            $template = "components/common/branch-info-modal.twig";
        else
            $template = "components/common/branch-info-panel.twig";
...
//            $template = "components/common/company-info-panel.twig";
        ...
        $hidden_fields = [];
        foreach ($fields as $field){
            if ($this->_app->user->checkAccess("update_branch_setting", [ "property" => $field]))
                $show_fields[] = $field;
            else if ($this->_app->user->checkAccess("view_branch_setting", [ "property" => $field]))
                $disabled_fields[] = $field;
            else
                $hidden_fields[] = $field;
        }
        ...
        // Load validator rules
        $schema = new \Fortress\RequestSchema($this->_app->config('schema.path') . "/forms/branch-update.json");
        $this->_app->jsValidator->setSchema($schema);

        $this->_app->render($template, [
            "box_id" => $get['box_id'],
            "box_title" => "Edit Branch",
            "submit_button" => "Update Branch",
            "form_action" => $this->_app->site->uri['public'] . "/branch/b/$branch_id",
            "branch" => $branch,
            "fields" => [
                "disabled" => $disabled_fields,
                "hidden" => $hidden_fields
...
public function formBranchCreate(){

    if (!$this->_app->user->checkAccess('create_branch')){
            $this->_app->notFound();
        }
        // Get the user to edit
//        $target_user = User::find($user_id);
...
//            $template = "components/common/company-info-panel.twig";
        ...
        $this->_app->render($template, [
            "box_id" => $get['box_id'],
            "box_title" => "Create Branch",
            "submit_button" => "Create Branch",
            "form_action" => $this->_app->site->uri['public'] . "/branches",
            "branch" => $branch,
            "fields" => [
                "disabled" => $disabled_fields,
                "hidden" => $hidden_fields
...
//            "themes" => $theme_list,
     ...
     */
        ...
        // Load the request schema
        $requestSchema = new \Fortress\RequestSchema($this->_app->config('schema.path') . "/forms/branch-create.json");
...
//        }
        ...
        //     }
        // }
        $branch->company_id = $this->_app->user->company_id;
        $branch->status = 1;
        $branch->lat = 1;
        $branch->lng = 1;
        // $branch->last_modified = NULL;
        $ms->addMessageTranslated("success", "GROUP_UPDATE", ["name" => $branch->name]);
        $branch->save();
...
// exit();
        ...
        // Load the request schema
        $requestSchema = new \Fortress\RequestSchema($this->_app->config('schema.path') . "/forms/branch-update.json");
...
//        $group = Group::find($company_id);
        ...
        foreach ($post as $name => $value) {
            if (isset($branch->$name) && $post[$name] != $branch->$name){
                // Check authorization
//                if (!$this->_app->user->checkAccess('update_branch_setting')){
//                    $ms->addMessageTranslated("danger", "ACCESS_DENIED");
//                    $this->_app->halt(403);
//                }
            } else if (!isset($branch->$name)) {
...
//        }
    ...
    public function deleteBranch($branch_id){
        ...
        // Check authorization
        if (!$this->_app->user->checkAccess('delete_branch', ['branch' => $branch])){
//         if(true){
            $ms->addMessageTranslated("danger", "ACCESS_DENIED");
            $this->_app->halt(403);
        }
...
//        }
        ...
        $ms->addMessageTranslated("success", "GROUP_DELETION_SUCCESSFUL", ["name" => $branch->name]);
        $branch->delete();       // TODO: implement Group function
        unset($branch);
    }
