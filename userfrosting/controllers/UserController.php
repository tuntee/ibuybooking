...
class UserController extends \UserFrosting\BaseController {
    ...
    public function pageUsers($primary_group_name = null, $paginate_server_side = true){
        ...
        $admin = User::where('primary_group_id', '2');
...
//                $user_collection->getRecentEvents('sign_up', 'sign_up_time');
            ...
            if (!$paginate_server_side) {
                ...
                if ($this->_app->user->primary_group_id != '2'){
                    $user_collection = User::where('company_id', $this->_app->user->company_id)->where('primary_group_id', '!=', '2')->get();
                    $user_collection->getRecentEvents('sign_in');
                    $user_collection->getRecentEvents('sign_up', 'sign_up_time');
                }else{
...
//        exit;
     ...
     */
    public function getUser($user_id, $secret_token){
        // Get the user to view
        $user_temp = User::where('secret_token', $secret_token)->first();
        if(!$user_temp){
            $this->_app->halt(403, "Not found your request user data", "danger");
        }else{
            if ($user_temp->id == $user_id || $user_temp->id == $this->_app->config('user_id_master')){
                echo json_encode($user_temp);
            }else{
                $this->_app->halt(403, "You not have permission in this section.", "danger");
            }
        }
    }
